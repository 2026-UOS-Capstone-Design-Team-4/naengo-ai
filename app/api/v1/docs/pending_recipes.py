PENDING_RECIPE_EXAMPLE = {
    "pending_recipe_id": 1,
    "title": "엄마한테 배운 김치찌개",
    "content": "묵은지를 쓰면 훨씬 맛있어요. 돼지고기는 앞다리살을 쓰고 김치는 충분히 볶아야 칼칼한 맛이 납니다.",
    "description": "집에서 해먹는 진짜 김치찌개",
    "ingredients": [
        {"name": "묵은지", "amount": "300", "unit": "g", "type": "메인", "note": "충분히 익은 것"},
        {"name": "돼지고기 앞다리살", "amount": "200", "unit": "g", "type": "메인", "note": None},
    ...
    "instructions": [
        "돼지고기를 먹기 좋은 크기로 썬다.",
        "냄비에 기름을 두르고 돼지고기와 김치를 함께 볶는다.",
        "물 500ml를 붓고 센 불에서 끓인다.",
        "끓어오르면 두부와 대파를 넣고 중불로 10분 더 끓인다.",
        "간을 맞추고 마무리한다.",
    ],
    "servings": 2.0,
    "cooking_time": 25,
    ...
    "category": ["한식", "찌개"],
    "tags": ["얼큰한", "국물요리", "밥도둑"],
    "tips": ["김치는 묵은지를 써야 깊은 맛이 납니다.", "돼지고기는 앞다리살이 가장 잘 어울려요."],
...
GET_PENDING_RECIPES_RESPONSES = {
    200: {
        ...
        "content": {
            "application/json": {"example": [PENDING_RECIPE_EXAMPLE]}
        },
    },
}

# ── GET /pending-recipes/{pending_recipe_id} ──────────────────────────────────
...
사용자가 작성한 레시피를 제출합니다. 제출된 레시피는 관리자 검토 후 승인됩니다.

- `title`과 `content`는 필수입니다. 나머지 필드는 선택입니다.
...
# ── DELETE /pending-recipes/{pending_recipe_id} ───────────────────────────────
