...
- 제목은 짧고 검색하기 쉬운 명사형으로 쓴다.
  예: "김치 두부찌개", "버섯 간장볶음"
- 제목에는 사용자가 검색할 핵심 키워드를 남긴다.
  핵심 주재료, 조리 형태(볶음, 찌개, 구이, 무침 등), 대표 소스나 맛이
  레시피를 구분하는 정보라면 제목에 포함한다.
- 제목을 짧게 만들더라도 핵심 재료명을 지나치게 일반화하지 않는다.
  예: "두부 요리"보다 "두부 간장조림"처럼 구체적으로 쓴다.
- 요리의 특색이 뚜렷한 경우, 맛·식감·조리 특성을 나타내는 꾸미는 말이나
  일반적이지 않은 특이한 재료를 제목에 더할 수 있다.
  예: "바삭한 두부 간장조림", "매콤한 김치 두부찌개", "들깨 미역국", "흑마늘 닭볶음탕"
  특색이 없거나 재료·조리법만으로 충분히 표현되면 생략한다.
